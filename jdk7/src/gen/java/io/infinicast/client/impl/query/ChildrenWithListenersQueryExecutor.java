...
public class ChildrenWithListenersQueryExecutor extends BaseQueryExecutor  {
    ...
    public void getChildrenWithListeners(final QuadConsumer<ICError, ArrayList<IPathAndCount>, IAPathContext, Integer> callback, int start, int limit) {
        ChildrenWithListenersQueryExecutor self = this;
        ...
        super._messageManager.sendMessageWithResponse(Connector2EpsMessageType.GetMatchingPathsWithListeners, super._path, settings, new DMessageResponseHandler() {
                ...
                if (!(checkIfHasErrorsAndCallHandlersNew(err, new CompleteCallback() {
                    ...
                    if ((array != null)) {
                        ...
                        for (JToken ob : array) {
                            ...
                            ele.setCount(count);
                            ele.setPath(_connector.getObjectStateManager().getOrCreateLocalObject(path));
                            resultList.add(ele);
                        }
                        callback.accept(null, resultList, getPathContext(_path), json.getInt("count"));
                        ;
                    }
                    else {
